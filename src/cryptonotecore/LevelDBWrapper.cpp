...
LevelDBWrapper::LevelDBWrapper(std::shared_ptr<Logging::ILogger> logger):
    logger(logger, "\033[35m[DB]\033[0m   LevelDBWrapper"),
...
void LevelDBWrapper::init(const DataBaseConfig &config)
{
    ...
    logger(INFO) << "\033[35m[DB]\033[0m   Daemon uses kSnappyCompression...";
    dbOptions.compression = leveldb::kSnappyCompression;
#else
    logger(INFO) << "\033[35m[DB]\033[0m   Daemon uses kNoCompression...";
    ...
    logger(INFO) << "\033[35m[DB]\033[0m   Opening DB in " << dataDir;

    if (true == status.ok())
    {
        logger(INFO) << "\033[35m[DB]\033[0m   DB opened in " << dataDir;
    }
    else if (!status.ok() && status.IsInvalidArgument())
    {
        logger(INFO) << "\033[35m[DB]\033[0m   DB not found in " << dataDir << ". Creating new DB...";
        dbOptions.create_if_missing = true;
        leveldb::Status status = leveldb::DB::Open(dbOptions, dataDir, &dbPtr);
        if (!status.ok())
        {
            logger(ERROR) << "\033[35m[DB]\033[0m   DB Error. DB can't be created in " << dataDir << ". Error: " << status.ToString();
            throw std::system_error(make_error_code(CryptoNote::error::DataBaseErrorCodes::INTERNAL_ERROR));
        }
    }
    else if (status.IsIOError())
    {
        logger(ERROR) << "\033[35m[DB]\033[0m   DB Error. DB can't be opened in " << dataDir << ". Error: " << status.ToString();
...
void LevelDBWrapper::shutdown()
{
    ...
    logger(INFO) << "\033[35m[DB]\033[0m   Closing DB.";
...
void LevelDBWrapper::destroy(const DataBaseConfig &config)
{
    ...
    logger(WARNING) << "\033[35m[DB]\033[0m   Destroying DB in " << dataDir;

    leveldb::Status status = leveldb::DestroyDB(dataDir, leveldb::Options());

    if (status.ok())
    {
        logger(WARNING) << "\033[35m[DB]\033[0m   DB destroyed in " << dataDir;
    }
    else
    {
        logger(ERROR) << "\033[35m[DB]\033[0m   DB Error. DB can't be destroyed in " << dataDir << ". Error: " << status.ToString();
        throw std::system_error(make_error_code(CryptoNote::error::DataBaseErrorCodes::INTERNAL_ERROR));
    }
}
...
std::error_code LevelDBWrapper::write(IWriteBatch &batch, bool sync)
{
    ...
    if (!status.ok())
    {
        logger(ERROR) << "\033[35m[DB]\033[0m   Can't write to DB. " << status.ToString();
