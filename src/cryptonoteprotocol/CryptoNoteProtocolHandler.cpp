...
#undef HANDLE_NOTIFY
    ...
    bool CryptoNoteProtocolHandler::on_connection_synchronized()
    {
        ...
        if (m_synchronized.compare_exchange_strong(val_expected, true))
        {
            ...
            logger(INFO, BRIGHT_RED) << "╔════[ " + std::string(CryptoNote::CRYPTONOTE_NAME)  + " node is ready! ]══════════════╗" << ENDL;
            logger(INFO, WHITE) << BRIGHT_RED << "║" << WHITE << " You are now ready to use the " + std::string(CryptoNote::CRYPTONOTE_NAME)  + " Node! " << BRIGHT_RED << "║" << ENDL;
            //logger(INFO, WHITE) << "" << ENDL;
            logger(INFO, BRIGHT_RED) << "╚══════════════════════════════════════════╝" << ENDL << ENDL;
        ...
        CryptoNoteProtocolHandler::updateObservedHeight(uint32_t peerHeight, const CryptoNoteConnectionContext &context)
    {
...

        {
            ...
            if (peerHeight > m_blockchainHeight)
            {
                ...
                logger(Logging::INFO, Logging::BRIGHT_RED) << "Highest block is " << peerHeight;
            }
        }
