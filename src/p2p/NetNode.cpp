...
namespace
{
    ...
    void addPortMapping(Logging::LoggerRef &logger, uint32_t port)
    {
        ...
        logger(INFO) << "\033[34m[CORE]\033[0m Attempting to add IGD port mapping.";
        int result;
        ...
        if (result != 0)
        {
            if (result == 1)
            {
                ...
                if (UPNP_AddPortMapping(
                    ...
                    != 0)
                {
                    logger(ERROR) << "\033[34m[CORE]\033[0m UPNP_AddPortMapping failed.";
                }
                else
                {
                    logger(INFO) << "\033[34m[CORE]\033[0m Added IGD port mapping.";
                }
            }
            else if (result == 2)
            {
                logger(INFO) << "\033[34m[CORE]\033[0m IGD was found but reported as not connected.";
            }
            else if (result == 3)
            {
                logger(INFO) << "\033[34m[CORE]\033[0m UPnP device was found but not recognized as IGD.";
            }
            else
            {
                logger(ERROR) << "\033[34m[CORE]\033[0m UPNP_GetValidIGD returned an unknown result code.";
            }
        ...
        {
            logger(INFO) << "\033[34m[CORE]\033[0m No IGD was found.";
        }
    }
...
namespace CryptoNote
{
    ...
    bool P2pConnectionContext::pushMessage(P2pMessage &&msg)
    {
        ...
        {
            logger(DEBUGGING) << "[DEBUG] " << *this << "Write queue overflows. Interrupt connection";
            interrupt();
            return false;
        }
    ...
    {
        logger(DEBUGGING) << "[DEBUG] " << *this << "Interrupt connection";
...
#undef INVOKE_HANDLER
    ...
    bool NodeServer::init_config()
    {
        try
        {
            ...
            catch (const std::exception &e)
            {
                logger(ERROR, BRIGHT_RED)
                    << "\033[34m[CORE]\033[0m Failed to load config from file '" << state_file_path << "': " << e.what();
            }
        ...
        {
            logger(ERROR, BRIGHT_RED) << "\033[34m[CORE]\033[0m init_config failed: " << e.what();
            return false;
        }
        return true;
    }
    ...
    bool NodeServer::make_default_config()
    {
        ...
        logger(INFO, BRIGHT_WHITE) << "\033[91m[P2P]\033[0m  Generated new peer ID: " << m_config.m_peer_id;
        return true;
    }
    ...
    bool NodeServer::append_net_address(std::vector<NetworkAddress> &nodes, const std::string &addr)
    {
        ...
        {
            logger(ERROR, BRIGHT_RED) << "\033[91m[P2P]\033[0m  Failed to parse seed address from string: '" << addr << '\'';
            return false;
        }
        ...
        try
        {
            ...
            logger(TRACE) << "\033[91m[P2P]\033[0m  Added seed node: " << nodes.back() << " (" << host << ")";
        }
        catch (const std::exception &e)
        {
            logger(ERROR, BRIGHT_YELLOW) << "\033[91m[P2P]\033[0m  Failed to resolve host name '" << host << "': " << e.what();
            return false;
        }
    ...
    bool NodeServer::init(const NetNodeConfig &config)
    {
        ...
        {
            logger(ERROR, BRIGHT_RED) << "\033[34m[CORE]\033[0m Failed to handle command line";
            return false;
        }
        ...
        if (!init_config())
        {
            logger(ERROR, BRIGHT_RED) << "\033[34m[CORE]\033[0m Failed to init config.";
            return false;
        }

        if (!m_peerlist.init(m_allow_local_ip))
        {
            logger(ERROR, BRIGHT_RED) << "\033[34m[CORE]\033[0m Failed to init peerlist.";
            return false;
        }
        ...
        // try to bind
        logger(INFO) << "\033[91m[P2P]\033[0m  Binding on " << m_bind_ip << ":" << m_port;
        ...
        logger(INFO) << "\033[91m[P2P]\033[0m  Net service bound on " << m_bind_ip << ":" << m_listeningPort;

        if (m_external_port)
        {
            logger(INFO) << "\033[91m[P2P]\033[0m  External port defined as " << m_external_port;
        }
    ...
    bool NodeServer::run()
    {
        logger(INFO) << "\033[34m[CORE]\033[0m Starting Node...";
        ...
        logger(INFO) << "\033[34m[CORE]\033[0m Stopping NodeServer and its " << m_connections.size() << " connections...";
        safeInterrupt(m_workingContextGroup);
        m_workingContextGroup.wait();

        logger(INFO) << "\033[34m[CORE]\033[0m NodeServer loop stopped";
        return true;
    }
    ...
    bool NodeServer::store_config()
    {
        try
        {
            ...
            {
                logger(INFO) << "\033[34m[CORE]\033[0m Failed to create data directory: " << m_config_folder;
                return false;
            }
            ...
            if (p2p_data.fail())
            {
                logger(INFO) << "\033[34m[CORE]\033[0m Failed to save config to file " << state_file_path;
                return false;
            };
        ...
        {
            logger(WARNING) << "\033[34m[CORE]\033[0m store_config failed: " << e.what();
        }
    ...
    bool NodeServer::handshake(CryptoNote::LevinProtocol &proto, P2pConnectionContext &context, bool just_take_peerlist)
    {
        ...
        if (!proto.invoke(COMMAND_HANDSHAKE::ID, arg, rsp))
        {
            logger(Logging::DEBUGGING)
                << context
                << "\033[34m[CORE]\033[0m A daemon on the network has departed. MSG: Failed to invoke COMMAND_HANDSHAKE, closing connection.";
            return false;
        }
        ...
        {
            logger(Logging::DEBUGGING) << context << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE Failed, wrong network! ("
        ...
        {
            logger(Logging::DEBUGGING) << context << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE Failed, peer is wrong version! ("
        ...
        else if ((rsp.node_data.version - CryptoNote::P2P_CURRENT_VERSION) >= CryptoNote::P2P_UPGRADE_WINDOW)
        {
            logger(Logging::WARNING) << context
                                     << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE Warning, your software may be out of date. Please visit: "
        ...
        if (!handle_remote_peerlist(rsp.local_peerlist, rsp.node_data.local_time, context))
        {
            logger(Logging::ERROR) << context
                                   << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE: failed to handle_remote_peerlist(...), closing connection.";
            return false;
        }
        ...
        {
            logger(Logging::TRACE) << context << "\033[91m[P2P]\033[0m  Connection to self detected, dropping connection";
            return false;
        }

        logger(Logging::DEBUGGING) << context << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE INVOKED OK";
        return true;
    }
    ...
    bool NodeServer::handleTimedSyncResponse(const BinaryArray &in, P2pConnectionContext &context)
    {
        ...
        {
            logger(Logging::ERROR) << context
                                   << "\033[91m[P2P]\033[0m  COMMAND_TIMED_SYNC: failed to handle_remote_peerlist(...), closing connection.";
            return false;
        }
    ...
    bool NodeServer::try_to_connect_and_handshake_with_new_peer(
        ...
        bool white)
    {
        logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Connecting to " << na << " (white=" << white << ", last_seen: "
        ...
        try
        {
            ...
            try
            {
                ...
                System::Context<> timeoutContext(m_dispatcher, [&] {
                    ...
                    logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Connection to " << na << " timed out, interrupt it";
            ...
            {
                logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Connection timed out";
                return false;
            }
            ...
            try
            {
                ...
                System::Context<> timeoutContext(m_dispatcher, [&] {
                    ...
                    logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Handshake with " << na << " timed out, interrupt it";
                    safeInterrupt(handshakeContext);
                });

                if (!handshakeContext.get())
                {
                    logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Failed to HANDSHAKE with peer " << na;
                    return false;
                }
            }
            catch (System::InterruptedException &)
            {
                logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Handshake timed out";
                return false;
            }

            if (just_take_peerlist)
            {
                logger(Logging::DEBUGGING, Logging::BRIGHT_GREEN) << ctx << "\033[91m[P2P]\033[0m  CONNECTION HANDSHAKED OK AND CLOSED.";
                return true;
            }
        ...
        {
            logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Connection process interrupted";
            throw;
        }
        catch (const std::exception &e)
        {
            logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Connection to " << na << " failed: " << e.what();
        }
    ...
    bool NodeServer::make_new_connection_from_peerlist(bool use_white_list)
    {
        ...
        while (rand_count < (max_random_index + 1) * 3 && try_count < 10 && !m_stop)
        {
            ...
            {
                logger(ERROR, BRIGHT_RED) << "\033[91m[P2P]\033[0m  random_starter_index < peers_local.size() failed!!";
                return false;
            }
            ...
            if (!(r))
            {
                logger(ERROR, BRIGHT_RED) << "\033[91m[P2P]\033[0m  Failed to get random peer from peerlist(white:" << use_white_list << ")";
                return false;
            }
            ...
            logger(DEBUGGING) << "\033[91m[P2P]\033[0m  Selected peer: " << pe.id << " " << pe.adr << " [white=" << use_white_list
                              << "] last_seen: "
    ...
    bool NodeServer::connections_maker()
    {
        ...
        if (!m_peerlist.get_white_peers_count() && m_seed_nodes.size())
        {
            ...
            while (true)
            {
                ...
                {
                    logger(ERROR) << "\033[91m[P2P]\033[0m  Failed to connect to any of seed peers, continuing without seeds";
                    break;
                }
    ...
    bool NodeServer::idle_worker()
    {
        ...
        {
            logger(DEBUGGING) << "\033[34m[CORE]\033[0m exception in idle_worker: " << e.what();
        }
        return true;
    }
    ...
    bool NodeServer::fix_time_delta(std::list<PeerlistEntry> &local_peerlist, time_t local_time, int64_t &delta)
    {
        ...
        BOOST_FOREACH (PeerlistEntry &be, local_peerlist)
        {
            ...
            {
                logger(DEBUGGING) << "\033[91m[P2P]\033[0m  FOUND FUTURE peerlist for entry " << be.adr << " last_seen: " << be.last_seen
...
//-----------------------------------------------------------------------------------
    ...
    bool NodeServer::check_trust(const proof_of_trust &tr)
    {
        ...
        {
            logger(ERROR) << "\033[91m[P2P]\033[0m  check_trust failed to check time conditions, local_time=" << local_time
                          << ", proof_time=" << tr.time;
            return false;
        }
        ...
        {
            logger(ERROR) << "\033[91m[P2P]\033[0m  check_trust failed to check time conditions, last_stat_request_time="
        ...
        {
            logger(ERROR) << "\033[91m[P2P]\033[0m  check_trust failed: peer_id mismatch (passed " << tr.peer_id << ", expected "
                          << m_config.m_peer_id << ")";
            return false;
        }
        ...
        {
            logger(ERROR) << "\033[91m[P2P]\033[0m  check_trust failed: sign check failed";
            return false;
        }
    ...
    bool NodeServer::try_ping(basic_node_data &node_data, P2pConnectionContext &context)
    {
        ...
        try
        {
            ...
            System::Context<> timeoutContext(m_dispatcher, [&] {
                ...
                logger(DEBUGGING) << context << "\033[91m[P2P]\033[0m  Back ping timed out" << ip << ":" << port;
            ...
            {
                logger(DEBUGGING) << context << "\033[91m[P2P]\033[0m  Back ping invoke wrong response \"" << rsp.status << "\" from" << ip
        ...
        {
            logger(DEBUGGING) << context << "\033[91m[P2P]\033[0m  Back ping connection to " << ip << ":" << port << " failed: " << e.what();
            return false;
        }
        ...
        P2pConnectionContext &context)
    {
        ...
        {
            logger(Logging::ERROR) << context << "\033[91m[P2P]\033[0m  Failed to process_payload_sync_data(), dropping connection";
            context.m_state = CryptoNoteConnectionContext::state_shutdown;
            return 1;
        }
        ...
        logger(Logging::TRACE) << context << "\033[91m[P2P]\033[0m  COMMAND_TIMED_SYNC";
        return 1;
    }
    //-----------------------------------------------------------------------------------
        ...
        P2pConnectionContext &context)
    {
        ...
        {
            logger(Logging::DEBUGGING) << context << "\033[91m[P2P]\033[0m  WRONG NETWORK AGENT CONNECTED! id=" << arg.node_data.network_id;
            context.m_state = CryptoNoteConnectionContext::state_shutdown;
            return 1;
        }
        ...
        {
            logger(Logging::DEBUGGING) << context << "\033[91m[P2P]\033[0m  UNSUPPORTED NETWORK AGENT VERSION CONNECTED! version="
        ...
        {
            logger(Logging::WARNING) << context << "\033[34m[CORE]\033[0m Our software may be out of date. Please visit: "
                                     << CryptoNote::LATEST_VERSION_URL << " for the latest version.";
        }

        if (!context.m_is_income)
        {
            logger(Logging::ERROR) << context << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE came not from incoming connection";
            context.m_state = CryptoNoteConnectionContext::state_shutdown;
            return 1;
        }
        ...
        {
            logger(Logging::ERROR) << context
                                   << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE came, but seems that connection already have associated "
        ...
        if (!m_payload_handler.process_payload_sync_data(arg.payload_data, context, true))
        {
            logger(Logging::ERROR)
                << context
                << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE came, but process_payload_sync_data returned false, dropping connection.";
            context.m_state = CryptoNoteConnectionContext::state_shutdown;
            return 1;
        }
        ...
        if (arg.node_data.peer_id != m_config.m_peer_id && arg.node_data.my_port)
        {
            ...
            if (try_ping(arg.node_data, context))
            {
                ...
                logger(Logging::TRACE) << context << "\033[91m[P2P]\033[0m  BACK PING SUCCESS, "
        ...
        logger(Logging::DEBUGGING, Logging::BRIGHT_GREEN) << "\033[91m[P2P]\033[0m  COMMAND_HANDSHAKE";
        return 1;
    }
    //-----------------------------------------------------------------------------------
    ...
    int NodeServer::handle_ping(
        ...
        P2pConnectionContext &context)
    {
        logger(Logging::TRACE) << context << "\033[91m[P2P]\033[0m  COMMAND_PING";
    ...
    bool NodeServer::log_peerlist()
    {
        ...
        logger(INFO) << ENDL << "\033[91m[P2P]\033[0m  Peerlist white:" << ENDL << print_peerlist_to_string(pl_wite) << ENDL
    ...
    {
        logger(INFO) << "\033[91m[P2P]\033[0m  Connections: \r\n" << print_connections_container();
        return true;
    }
    //-----------------------------------------------------------------------------------
    ...
    {
        logger(TRACE) << context << "\033[91m[P2P]\033[0m  NEW CONNECTION";
    ...
    {
        logger(TRACE) << context << "\033[91m[P2P]\033[0m  CLOSE CONNECTION";
    ...
    void NodeServer::acceptLoop()
    {
        while (!m_stop)
        {
            ...
            {
                logger(DEBUGGING) << "[DEBUG] acceptLoop() is interrupted";
                break;
            }
            catch (const std::exception &e)
            {
                logger(DEBUGGING) << "[DEBUG] Exception in acceptLoop: " << e.what();
            }
        }

        logger(DEBUGGING) << "[DEBUG] acceptLoop finished";
    }

    void NodeServer::onIdle()
    {
        logger(DEBUGGING) << "[DEBUG] onIdle started";
        ...
        while (!m_stop)
        {
            ...
            {
                logger(DEBUGGING) << "[DEBUG] onIdle() is interrupted";
                break;
            }
            catch (std::exception &e)
            {
                logger(WARNING) << "[DEBUG] Exception in onIdle: " << e.what();
            }
        }

        logger(DEBUGGING) << "[DEBUG] onIdle finished";
    }
    ...
    void NodeServer::timeoutLoop()
    {
        try
        {
            while (!m_stop)
            {
                ...
                for (auto &kv : m_connections)
                {
                    ...
                    {
                        logger(DEBUGGING) << ctx << "[DEBUG] write operation timed out, stopping connection";
                        safeInterrupt(ctx);
                    }
                }
            }
        ...
        {
            logger(DEBUGGING) << "[DEBUG] timeoutLoop() is interrupted";
        }
        catch (std::exception &e)
        {
            logger(WARNING) << "[DEBUG] Exception in timeoutLoop: " << e.what();
        }
        catch (...)
        {
            logger(WARNING) << "[DEBUG] Unknown exception in timeoutLoop";
        }
    }
    ...
    void NodeServer::timedSyncLoop()
    {
        ...
        {
            logger(DEBUGGING) << "[DEBUG] timedSyncLoop() is interrupted";
        }
        catch (std::exception &e)
        {
            logger(WARNING) << "[DEBUG] Exception in timedSyncLoop: " << e.what();
        }

        logger(DEBUGGING) << "[DEBUG] timedSyncLoop finished";
    }
    ...
    void NodeServer::connectionHandler(const boost::uuids::uuid &connectionId, P2pConnectionContext &ctx)
    {
        ...
        System::Context<> context(m_dispatcher, [this, &connectionId, &ctx] {
            ...
            {
                logger(DEBUGGING) << ctx << "[DEBUG] connectionHandler() inner context is interrupted";
            }
            catch (std::exception &e)
            {
                logger(DEBUGGING) << ctx << "[DEBUG] Exception in connectionHandler: " << e.what();
            }
        ...
        {
            logger(DEBUGGING) << "[DEBUG] connectionHandler() is interrupted";
        }
        catch (std::exception &e)
        {
            logger(WARNING) << "\033[91m[P2P]\033[0m  connectionHandler() throws exception: " << e.what();
        }
        catch (...)
        {
            logger(WARNING) << "\033[91m[P2P]\033[0m  connectionHandler() throws unknown exception";
        }
    }

    void NodeServer::writeHandler(P2pConnectionContext &ctx)
    {
        logger(DEBUGGING) << "[DEBUG] " << ctx << "writeHandler started";
        ...
        try
        {
            ...
            for (;;)
            {
                ...
                {
                    logger(DEBUGGING) << "[DEBUG] " << ctx << "msg " << msg.type << ':' << msg.command;
                    switch (msg.type)
                    {
        ...
        {
            // connection stopped
            logger(DEBUGGING) << "[DEBUG] "<< ctx << "writeHandler() is interrupted";
        }
        catch (std::exception &e)
        {
            logger(DEBUGGING) << "[DEBUG] "<< ctx << "error during write: " << e.what();
            safeInterrupt(ctx); // stop connection on write error
        }

        logger(DEBUGGING) << "[DEBUG] "<< ctx << "writeHandler finished";
    }
    ...
    template<typename T> void NodeServer::safeInterrupt(T &obj)
    {
        ...
        {
            logger(WARNING) << "\033[34m[CORE]\033[0m interrupt() throws exception: " << e.what();
        }
        catch (...)
        {
            logger(WARNING) << "\033[34m[CORE]\033[0m interrupt() throws unknown exception";
        }
    }
