...
namespace WalletConfig
{
    ...
    const std::string_view addressPrefix = "VRM";

    /* Your coins 'Ticker', e.g. Monero = XMR, Bitcoin = BTC */
    const std::string ticker = "VRM";
    ...
    const std::string daemonName = "Verumd";

    /* The name to call this wallet */
    const std::string walletName = "Verum-Wallet";

    /* The name of service/walletd, the programmatic rpc interface to a
       wallet */
    const std::string walletdName = "Verum-Service";
    ...
    /* The default fee value to use with transactions (in ATOMIC units!) - 500 VRM */
    const uint64_t defaultFee = CryptoNote::parameters::MINIMUM_FEE;

    /* The minimum fee value to allow with transactions (in ATOMIC units!)  - 500 VRM */
