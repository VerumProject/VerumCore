...
// Copyright (c) 2024, The Verum Team
//
...
namespace CryptoNote
{
  namespace parameters
  {
    ...
    const uint64_t CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX                 = 0x634a9;                         /* Wallet addresses prefix: VRM */
    ...
    const uint64_t MONEY_SUPPLY                                            = UINT64_C(10'100'000'00000000);   /* 10'100'000 VRM */
    ...
    const char GENESIS_COINBASE_TX_HEX[] = 
      "010a01ff000180c6868f01029b2e4c0281c0b02e7c53291a94d1d0cbff8883f8024f5142ee494ffbbd0880712101c5e6fb1fc991e1a7872d3aa07114d97e9c543d4fb4972ef3a42e919e1737096c";
    ...
    /* This is the unix timestamp of the first "mined" block (technically block 2, not the genesis block)
       You can get this value by doing "print_block 2" in Verumd. It is used to know what timestamp
       to import from when the block height cannot be found in the node or the node is offline. */
    const uint64_t GENESIS_BLOCK_TIMESTAMP                                 = 1727805600;                    /* 08:00 PM GMT+1, 1st of October 2024 */
    ...
    /* Fee to charge per byte of transaction. Will be applied in chunks, see
       ...
       - 5 VRM vs 5.12 VRM. You can read this as.. the fee per chunk
    ...
    const uint64_t ACCEPTABLE_FEE                                          = UINT64_C(100);       /* with this fee, tx is always accepted whatever size - 0.00000100 VRM */
    const uint64_t ACCEPTABLE_FEE_V1                                       = UINT64_C(100);       /* with this fee, tx is always accepted whatever size - 0.00000100 VRM */
    ...
    const uint32_t MIXIN_LIMITS_V2_HEIGHT                                  = 500000000;
    ...
    /* 10,000,000 VRM -> Max supply / mixin+1 outputs                 */
    /* This is enforced on the daemon side. An output > 30,000,000 causes an invalid block.   */
    const uint64_t MAX_OUTPUT_SIZE_NODE                                    = 10'000'000'00000000;
  ...
  const char    CRYPTONOTE_NAME[]                                          = "Verum";
  ...
  const int      P2P_DEFAULT_PORT                                          = 14000;           // P2P Port
  const int      RPC_DEFAULT_PORT                                          = 14001;           // RPC Port
  const int      SERVICE_DEFAULT_PORT                                      = 14002;           // Service Port
  ...
  const char LATEST_VERSION_URL[]                                          = "https://github.com/VerumProject/VerumCore/releases";
  const std::string LICENSE_URL                                            = "https://github.com/VerumProject/VerumCore/blob/master/LICENSE";

  const static boost::uuids::uuid CRYPTONOTE_NETWORK = {
    {0x3f, 0x82, 0xd9, 0x6b, 0xe7, 0x4a, 0x15, 0xc9, 0x5d, 0x1e, 0x92, 0xaf, 0x38, 0x73, 0xbc, 0x09}
  };
