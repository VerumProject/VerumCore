...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        logger(INFO, BRIGHT_RED) << getProjectCLIHeader() << std::endl;

        logger(INFO) << "\033[34m[CORE]\033[0m Program Working Directory: " << cwdPath;
        ...
        {
            logger(INFO) << "\033[34m[CORE]\033[0m Rewinding blockchain to: " << config.rewindToHeight << std::endl;
            ...
            logger(INFO) << "\033[34m[CORE]\033[0m Blockchain rewound to: " << config.rewindToHeight << std::endl;
        }
        ...
        if (use_checkpoints)
        {
            logger(INFO) << "\033[34m[CORE]\033[0m Loading Checkpoints for faster initial sync...";
            if (config.checkPoints == "default")
            {
                ...
                logger(INFO) << "\033[34m[CORE]\033[0m Loaded " << CryptoNote::CHECKPOINTS.size() << " default checkpoints";
            }
            else
            {
                ...
                if (!results)
                {
                    throw std::runtime_error("\033[34m[CORE]\033[0m Failed to load checkpoints");
                }
            }
        }
        ...
        logger(INFO) << "\033[34m[CORE]\033[0m Initializing core...";
        ...
        logger(INFO) << "\033[34m[CORE]\033[0m Core initialized OK";
        ...
        logger(INFO) << "\033[91m[P2P]\033[0m  Initializing P2P server...";
        if (!p2psrv->init(netNodeConfig))
        {
            logger(ERROR, BRIGHT_RED) << "\033[91m[P2P]\033[0m  Failed to initialize P2P server.";
            return 1;
        }

        logger(INFO) << "\033[91m[P2P]\033[0m  P2P server initialized OK";

        // Fire up the RPC Server
        logger(INFO) << "\033[32m[RPC]\033[0m  Starting core RPC server on address " << config.rpcInterface << ":" << config.rpcPort;
        ...
        logger(INFO) << "\033[91m[P2P]\033[0m  Starting P2P net loop...";
        p2psrv->run();
        logger(INFO) << "\033[91m[P2P]\033[0m  P2P net loop stopped";

        dch.stop_handling();

        // stop components
        logger(INFO) << "\033[32m[RPC]\033[0m  Stopping core RPC server...";
        rpcServer.stop();

        // deinitialize components
        logger(INFO) << "\033[91m[P2P]\033[0m  Deinitializing P2P...";
        p2psrv->deinit();
    ...
    {
        logger(ERROR, BRIGHT_RED) << "\033[34m[CORE]\033[0m Exception: " << e.what();
        return 1;
    }

    logger(INFO) << "\033[34m[CORE]\033[0m Node stopped.";
    return 0;
}
